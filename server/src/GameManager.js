...
export class GameManager {
  ...
  createSession({ gameId = 'us-you-be-the-president', pin, requireApproval = config.requireApprovalDefault }) {
  ...
  // mode: 'versus' (wait to be paired) | 'solo' (play alone / vs the computer).
  // nation: the solo joiner's chosen side — the tribe/president they picked
  // (e.g. 'houston' | 'lamar'). Solo-only games (no versus mode) force solo.
  joinStudent({ joinCode, nickname, mode = 'solo', nation = null }) {
    const session = this.registry.get(joinCode);
    ...
    const soloOnly = !game.modes.includes('versus');
    const effMode = soloOnly || mode === 'solo' ? 'solo' : 'versus';
    ...
    const student = {
      ...
      mode: effMode,
      nation: effMode === 'solo' && game.sides.includes(nation) ? nation : null,
      matchId: null,
      status: 'not_started',
      accuracy: null,
      connected: true,
    ...
    // The roster snapshot must be built AFTER this, so the teacher's very first
    // lobby:update already shows 'in_progress' instead of a stale 'not_started'
    // for the rest of the match (shared-engine bug, fixed here too).
  ...
  _matchBeginPayload(game, match, side, opponentName) {
    ...
    // Solo games with no AI opponent (Hold the Line; President of the Republic)
    // have no rival side — rivalMeters is null in that case.
    const rival = rivalOf(game, match, side);
    return {
      ...
      // Branching games (Dust Bowl): the live path, so a rejoin mid-story
      // restores the STAY/GO chip. null for non-branching games.
      path: state.sides[side].path ?? null,
    };
  }
  ...
  submitMove({ joinCode, studentId, move }) {
    ...
    if (!match.solo) {
      const oppId = match.sideToStudent[otherSide(game, side)];
      if (oppId !== 'AI') {
  ...
  _afterMove(session, game, match, side, res) {
    ...
    // chapter using its nation's real (historically right) strategy. Solo games
    // with no AI opponent (Hold the Line; President of the Republic) have no
    // rival side, so rivalOf() is null and there's nothing to drive.
    if (match.solo && res.chapterDone) {
      const aiSide = rivalOf(game, match, side);
    ...
    if (!match.solo) {
      ...
      state.chapterIndex = Math.min(chapterAt(game, sa.cursor), chapterAt(game, sb.cursor));
    } else {
      state.chapterIndex = chapterAt(game, state.sides[side].cursor);
    }
  ...
  _driveAIChapter(game, match, aiSide) {
    ...
    // Drive the AI to the end of its CURRENT chapter (chapters may vary in length).
    const startChapter = chapterAt(game, ss.cursor);
    while (ss.cursor < game.totalActions && chapterAt(game, ss.cursor) === startChapter) {
  ...
  _completeMatch(session, game, match) {
    ...
    for (const [side, sid] of players) {
      ...
      const rivalSide = rivalOf(game, match, side);
      ...
      emits.push(toStudent(sid, 'match:end', {
        ...
        you: mine, // holdScore/claimScore, meters, accuracy, ending, debrief, ...
  ...
  _replaceWithAI(session, match, goneStudentId) {
    ...
    // "The chapter the human has reached": their last played step's chapter,
    // rounded up past any completed chapter (general form of ceil(cursor/2)).
    const humanChapter = human.cursor === 0 ? 0 : chapterAt(game, human.cursor - 1) + 1;
    while (ai.cursor < game.totalActions &&
           chapterAt(game, ai.cursor) < humanChapter) {
  ...
  roster(session) {
    ...
    const students = [...session.students.values()].map((s) => {
      // Branching games (Dust Bowl) expose the chosen path live off the match
      // state; it rides along on the roster as info, never as a grade. Null for
      // non-branching games and for anyone not yet in a solo match.
      const m = s.matchId ? session.matches.get(s.matchId) : null;
      const path = m && m.solo ? (m.gameState.sides[m.side]?.path ?? null) : null;
      return {
        id: s.id,
        name: s.displayName,
        approved: s.approved,
    ...
    return {
      ...
      classAccuracy,
      meta: game.meta,
      // Class-wide accuracy PER CHAPTER, live across every match (in progress or
      // done) — lets a teacher see which chapter to reteach (spec: Tax Collector
      // vs. Tea Party §9). Every graded action carries its `chapter` index
      // (_stepGame.js); games without `chapterCount` simply get an empty array.
      chapterAccuracy: chapterAccuracy(game, session),
    };
  }
...
// Cursor → chapter index. Games with variable-length chapters (Ratify It!)
// expose chapterOf from the step-game factory; the floor(cursor/2) fallback is
// exactly what every older two-steps-per-chapter game used.
function chapterAt(game, cursor) {
  return game.chapterOf ? game.chapterOf(cursor) : Math.floor(cursor / 2);
}

// The rival side for RIVAL DISPLAY/AI purposes. Games that play solo with no AI
// opponent set soloRival:false — a single-role game (Hold the Line) or a variant
// game whose two sides never face each other (President of the Republic: you
// govern one president alone). Then there is no rival to compare against or drive.
function rivalOf(game, match, side) {
  if (match.solo && game.soloRival === false) return null;
  return otherSide(game, side);
}
...
// Class-wide accuracy per chapter, across every match in the session (live —
// not just completed ones). Returns an array of { count, average } indexed by
// chapter (0-based), length `game.chapterCount`; games without chapters get [].
function chapterAccuracy(game, session) {
  const n = game.chapterCount || 0;
  const buckets = Array.from({ length: n }, () => ({ points: 0, total: 0 }));
  for (const match of session.matches.values()) {
    for (const ss of Object.values(match.gameState.sides)) {
      for (const a of ss.actions || []) {
        const b = buckets[a.chapter];
        if (b) { b.points += a.points; b.total += 1; }
      }
    }
  }
  return buckets.map((b) => ({
    count: b.total,
    average: b.total ? Math.round((b.points / b.total) * 100) : null,
  }));
}
